...
- `ctx::CurlContext`: Stores the response received after executing the HTTP request.
"""
struct CurlRequest
    method::String
    url::String
...
function curl_setup_req(c::CurlClient, r::CurlRequest)
    ...
    curl_easy_setopt(c.curl_handle, CURLOPT_USERAGENT, "Curl/1.2.0")
...
Base.get(url; kw...)::CurlResponse = request("GET", url; kw...)
